...
Tests: `python tests/test_pipeline.py` — 17 tests (normaliser units + the 10 broken fixtures end to end).

## Review UI — Streamlit (recommended)

    pip install -r requirements-streamlit.txt
    streamlit run streamlit_app.py            # or: python -m streamlit run streamlit_app.py

Sidebar: data source, results folder, the "send draft BL" label, Run pipeline, exports. Tabs: Overview, Emails (click a
row for the SI-vs-BL table, source documents and page images), Review queue (pre-filled values, confirm OK / mismatch,
undo), Mismatch report (CSV). First launch may ask for an email in the terminal — just press Enter.
If pip cannot install Streamlit's dependencies on a very new Python (e.g. 3.14), use a Python 3.12/3.13 venv.

## Review UI — standard library only (no installs)

    python app.py                                  # data in this folder, results in ./out  -> http://localhost:8501
    python app.py --source http://localhost:8080   # organisers' docker dataset; enables "Submit to scoreboard"

Overview (counts, which fields differ, why cases escalated) · Emails (filter, click for SI-vs-BL table and the source
documents, page images for PDFs/scans) · Review queue (why it escalated, pre-filled values, enter corrected values /
confirm OK / confirm mismatch, undo) · Mismatch report (CSV). Decisions are saved to `out/resolutions.json` and re-applied
on every run, by the same code as the CLI. Standard library only; binds to 127.0.0.1.
