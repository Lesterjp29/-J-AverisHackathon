...
def test_streamlit_app_end_to_end():
    """Drive the Streamlit UI headlessly: run the pipeline, then resolve a blank value as a reviewer."""
    import tempfile
    import unittest
    try:
        from streamlit.testing.v1 import AppTest
    except ImportError:
        raise unittest.SkipTest("streamlit not installed (pip install -r requirements-streamlit.txt)")
    at = AppTest.from_file(str(ROOT / "streamlit_app.py"), default_timeout=120).run()
    at.sidebar.text_input[0].set_value(str(ROOT / "stress"))
    at.sidebar.text_input[1].set_value(tempfile.mkdtemp()).run()
    at.sidebar.button[0].click().run()
    assert not at.exception, [e.value for e in at.exception]
    assert {m.label: m.value for m in at.metric}["Emails processed"] == "10"
    assert "(3)" in at.tabs[4].label, at.tabs[4].label          # deleted BL, blank value, corrupt PDF
    key = lambda coll, k: next(w for w in coll if w.key == k)
    key(at.text_input, "v:email_t06_blank_value:bl:consignee").set_value("EAST BRIGHT FZ-LLC").run()
    key(at.button, "save:email_t06_blank_value").click().run()
    assert not at.exception and "(2)" in at.tabs[4].label, at.tabs[4].label
...
def _app_test(cwd=None, docker=False):
    import os
    import unittest
    try:
        from streamlit.testing.v1 import AppTest
    ...
    if docker:
        os.environ["SDOC_DOCKER"] = "1"
    if cwd:
        os.chdir(cwd)
    return AppTest.from_file(str(ROOT / "streamlit_app.py"), default_timeout=180).run()


def test_streamlit_quick_check_without_any_dataset():
    """What a teammate sees first in Docker: no inbox mounted. Quick check must still work; inbox tabs explain why."""
    import os
    import tempfile
    here = os.getcwd()
    try:
        empty = os.path.join(tempfile.mkdtemp(), "data")
        os.makedirs(empty)
        at = _app_test(empty, docker=True)
        assert not at.exception, [e.value for e in at.exception]
        assert any("No inbox dataset is connected" in i.value and "docker-compose.yml" in i.value for i in at.tabs[2].info), \
            [i.value[:160] for i in at.tabs[2].info]
        key = lambda coll, k: next(w for w in coll if w.key == k)
        key(at.button, "qc_s1").click().run()
        assert not at.exception and any("Mismatch found" in e.value and "Consignee" in e.value for e in at.tabs[0].error)
        key(at.button, "qc_s2").click().run()
        assert not at.exception and any("No mismatch detected" in x.value for x in at.tabs[0].success)
        key(at.button, "qc_s3").click().run()
        assert any("Mismatch found" in e.value for e in at.tabs[0].error)
        assert any("Draft reply" in x.label for x in at.tabs[0].expander)
    finally:
        os.chdir(here)


def test_streamlit_scan_tab_with_photos():
    _skip_without("cv2")
    import os
    import tempfile
    here = os.getcwd()
    try:
        ...
        at = _app_test(empty, docker=True)
        key = lambda coll, k: next(w for w in coll if w.key == k)
        key(at.button, "sc_match").click().run()
        assert not at.exception and any("No mismatch detected" in x.value for x in at.tabs[1].success), \
            ([e.value for e in at.tabs[1].error], [w.value for w in at.tabs[1].warning])
        key(at.button, "sc_mism").click().run()
        assert not at.exception and any("Mismatch found" in e.value and "Consignee" in e.value for e in at.tabs[1].error)
    finally:
        os.chdir(here)
